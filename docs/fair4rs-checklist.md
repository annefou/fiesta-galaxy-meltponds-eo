...
| FORRT Outcome's `Repository URL` field | Points to `https://github.com/annefou/fiesta-galaxy-meltponds-eo`. |
...
| GHCR Docker pull | `docker pull ghcr.io/annefou/fiesta-galaxy-meltponds-eo:latest` over HTTPS. |
...
- [ ] All `{{...}}` placeholder tokens substituted, **except `10.5281/zenodo.20943752`** which is allowed to remain until the first release mints the concept DOI (see line 167). Audit with: `grep -rln '{{[A-Z_]\+}}' . --include='*.md' --include='*.yml' --include='*.json' --include='*.cff' --include='*.toml' | while read f; do grep -oE '\{\{[A-Z_]+\}\}' "$f" | grep -qv '10.5281/zenodo.20943752' && echo "$f"; done` — should return nothing.
...
- [ ] If this is the first release: `CITATION.cff` `doi` field is left as `10.5281/zenodo.20943752` (the integration mints it on release; update afterwards).
...
- [ ] Docker image builds and runs the snakemake target via `docker run --rm ghcr.io/annefou/fiesta-galaxy-meltponds-eo:latest`.
